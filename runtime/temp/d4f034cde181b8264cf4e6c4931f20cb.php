    ...
    <div class="push_data">
        <div class="layui-row">
                ...
                <div class="layui-row layui-col-space3">
                    ...
                    <div class="layui-col-xs4">
                        <div class="layui-form-item remove_margin reset_item">
                            ...
                            <div class="layui-input-block">
                                <input type="text" class="layui-input" id="number" placeholder="请输入单据编号" value="<?php echo (isset($class['number']) && ($class['number'] !== '')?$class['number']:get_number('JFDHD')); ?>" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="set_btn">
                ...
                <?php else: if(empty($class['type']['nod']) || (($class['type']['nod'] instanceof \think\Collection || $class['type']['nod'] instanceof \think\Paginator ) && $class['type']['nod']->isEmpty())): if((get_root('exchange_edit'))): ?>
                            <button class="layui-btn layui-btn-normal" onclick="save(<?php echo isset($class['id'])?$class['id']:'0'; ?>);">保存单据</button>
                        <?php endif; if((get_root('exchange_auditing'))): ?>
                            <button class="layui-btn layui-btn-primary" onclick="auditing(<?php echo isset($class['id'])?$class['id']:'0'; ?>,true);">审核单据</button>
                        <?php endif; else: if((get_root('exchange_auditing'))): ?>
        ...
        <div class="layui-row layui-col-space12 top_12">
            <div class="layui-col-xs4">
                <div class="layui-form-item remove_margin">
                    <label class="layui-form-label">单据积分</label>
                    <div class="layui-input-block">
                        <input type="text" class="layui-input" id="total" placeholder="单据积分" disabled value="<?php echo isset($class['total'])?$class['total']:''; ?>" />
                    </div>
                </div>
            </div>
            <div class="layui-col-xs4">
                <div class="layui-form-item remove_margin ">
                    <label class="layui-form-label" onclick="get_total();">实际积分</label>
                    <div class="layui-input-block">
                        <input type="text" class="layui-input" id="actual" placeholder="实际积分 | 点击左侧快捷录入" value="<?php echo isset($class['actual'])?$class['actual']:''; ?>" />
                    </div>
                </div>
            </div>
            <div class="layui-col-xs4">
                <div class="layui-form-item remove_margin ">
                    <label class="layui-form-label">实收积分</label>
                    <div class="layui-input-block">
                        <input type="text" class="layui-input" id="integral" placeholder="请输入实收积分" value="<?php echo isset($class['integral'])?$class['integral']:''; ?>" />
                    </div>
                </div>
            </div>
            <div class="layui-col-xs4">
                <div class="layui-form-item remove_margin ">
                    <label class="layui-form-label">客户积分</label>
                    <div class="layui-input-block">
                        <input type="text" class="layui-input" id="customer_integral" placeholder="请先选择客户" value="<?php if(!(empty(\think\Request::instance()->get('id')) || ((\think\Request::instance()->get('id') instanceof \think\Collection || \think\Request::instance()->get('id') instanceof \think\Paginator ) && \think\Request::instance()->get('id')->isEmpty()))): ?><?php echo $class['customerinfo']['integral']; endif; ?>" disabled="disabled"/>
                    </div>
                </div>
            </div>
            <div class="layui-col-xs4">
...
<script type="text/javascript" charset="utf-8">
    ...
    var formfield=<?php echo get_formfield('exchange_main','jqgrid'); ?>;
    ...
    var plug_val=<?php if(empty($class) || (($class instanceof \think\Collection || $class instanceof \think\Paginator ) && $class->isEmpty())): ?>{nod_user:<?php echo get_selectpage('user',Session('is_user_id')); ?>}<?php else: ?>{nod_user:<?php echo get_selectpage('user',$class['user']); ?>,nod_customer:<?php echo get_selectpage('customer',$class['customer']); ?>}<?php endif; ?>;
