                ...
                <tbody>
                    <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                        <tr id="<?php echo $vo['id']; ?>" pid="<?php echo $vo['pid']; ?>">
                            <td><?php if($vo['pid'] == '0'): ?>顶级菜单<?php else: ?><?php echo $vo['pidinfo']['name']; endif; ?></td>
                            <td><?php echo $vo['name']; ?></td>
                            <td><?php echo $vo['type']['name']; ?></td>
                            <td><?php echo $vo['jump']['name']; ?></td>
                            <td><?php echo $vo['url']; ?></td>
                            <td><?php echo $vo['sort']; ?></td>
                            <td><?php echo $vo['root']; ?></td>
...
<script type="text/javascript" charset="utf-8">
    var ztree_data=<?php array_unshift($list,['id'=>0,'pid'=>0,'name'=>'顶级分类','open'=>'true']);echo json_encode(array_field($list,['id','pid','name','open'])); ?>;
</script>
...
<script src="/skin/js/develop/menu.js" type="text/javascript" charset="utf-8"></script>
</div>
