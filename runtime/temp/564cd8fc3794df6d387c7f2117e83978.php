    ...
    <div class="push_data">
        <div class="layui-row">
                ...
                <div class="layui-row layui-col-space3">
                    ...
                    <div class="layui-col-xs4">
                        <div class="layui-form-item remove_margin reset_item">
                            ...
                            <div class="layui-input-block">
                                <input type="text" class="layui-input" id="number" placeholder="请输入单据编号" value="<?php echo (isset($class['number']) && ($class['number'] !== '')?$class['number']:get_number('GHTHD')); ?>" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="set_btn">
                ...
                <?php else: if(empty($class['type']['nod']) || (($class['type']['nod'] instanceof \think\Collection || $class['type']['nod'] instanceof \think\Paginator ) && $class['type']['nod']->isEmpty())): if((get_root('repurchase_edit'))): ?>
                            <button class="layui-btn layui-btn-normal" onclick="save(<?php echo isset($class['id'])?$class['id']:'0'; ?>);">保存单据</button>
                        <?php endif; if((get_root('repurchase_auditing'))): ?>
                            <button class="layui-btn layui-btn-primary" onclick="auditing(<?php echo isset($class['id'])?$class['id']:'0'; ?>,true);">审核单据</button>
                        <?php endif; else: if((get_root('repurchase_auditing'))): ?>
        ...
        <div class="layui-row layui-col-space12 top_12">
            ...
            <div class="layui-col-xs4">
                <div class="layui-form-item remove_margin ">
                    ...
                    <div class="layui-input-block">
                        <input type="text" class="layui-input" id="money" placeholder="请输入实收金额" value="<?php echo isset($class['money'])?$class['money']:''; ?>" />
                    </div>
                </div>
            </div>
            <div class="layui-col-xs4">
...
<script type="text/javascript" charset="utf-8">
    ...
    var formfield=<?php echo get_formfield('repurchase_main','jqgrid'); ?>;
    ...
    var plug_val=<?php if(empty($class) || (($class instanceof \think\Collection || $class instanceof \think\Paginator ) && $class->isEmpty())): ?>{nod_user:<?php echo get_selectpage('user',Session('is_user_id')); ?>}<?php else: ?>{nod_user:<?php echo get_selectpage('user',$class['user']); ?>,nod_supplier:<?php echo get_selectpage('supplier',$class['supplier']); ?>,nod_account:<?php echo get_selectpage('account',$class['account']); ?>}<?php endif; ?>;
